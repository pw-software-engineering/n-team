class HotelOfferReviewEntry {
    ...
    createComponent() {
        ...
        reviewComponent.append(
            ...
            $("<p>").attr("class", "px-3 m-0").text(this.reviewData.content),
            $("<hr>").attr("class", "my-2").attr("style", "border-top: 1px dashed lightgray")
        );
    ...
    createReviewHeaderComponent() {
        ...
        var ratingBoxComponent = $("<div>").attr("align-self-start text-nowrap");
        var i = 0;
        for (; i < this.reviewData.rating; i++) {
            ratingBoxComponent.append('<i class="fas fa-star text-warning">');
        }
        for (; i < 5; i++) {
            ratingBoxComponent.append('<i class="far fa-star text-warning">');
        }
